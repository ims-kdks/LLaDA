...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self.use_cache(time_step):
            return x + self.__cache[f"delta{self.layer_id}"], None
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, time_step=time_step)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
        else:
